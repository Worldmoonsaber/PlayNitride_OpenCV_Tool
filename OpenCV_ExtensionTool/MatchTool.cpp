...
bool CMatchTool::CheckOverlapWithRotatedRect(vector<RotatedRect> vecRef,Size2f sz, s_SingleTargetMatch vec, int iMethod, double dMaxOverLap)
{
	...
	try
	{
		...
		RotatedRect rec = RotatedRect(vec.ptCenter_New, sz, vec.dMatchedAngle);

		for (int j = 0; j < vecRef.size(); j++)
		{
			vector<Point2f> vecInterSec;
			int iInterSecType = rotatedRectangleIntersection(rec, vecRef[j], vecInterSec);
			...
			else//交點 > 0
			{
				...
				else
				{
					...
					double dRatio = dArea / vecRef[j].size.area();
					//若大於最大交疊比例，選分數高的
...
void CMatchTool::_RoughMatch(int iTopLayer, vector<double> vecLayerScore, vector<Mat> vecMatSrcPyr, vector<vector<s_MatchParameter>>& vecVecMatchParameter, s_TemplData* pTemplData)
{

	//---從 -m_dToleranceAngle 到 m_dToleranceAngle
	//----此處假設全部的結果 都是相同角度
	double dAngleStep = atan(2.0 / max(pTemplData->vecPyramid[iTopLayer].cols, pTemplData->vecPyramid[iTopLayer].rows)) * R2D;


	double _lowerBound = -m_dToleranceAngle;
	double _upperBound = m_dToleranceAngle;

	double score = -999;
	double tmpAngle;


	if (m_dToleranceAngle != 0 && m_iMaxPos < 20)
	{
#pragma region 由上邊界往下找
		//---尋找 UpperBound

		while (true)
		{
			bool isMatch = _FindMatch_SpecificAngle(iTopLayer, vecLayerScore, vecMatSrcPyr, score, pTemplData, _upperBound);
			...
			if (abs(_upperBound - _lowerBound) < dAngleStep)
				break; //找這麼大圈依然找不到 可以視為 無法Match

			score = -999;
		}

		//---尋找 UpperBound
		tmpAngle = _upperBound;

		if (score != -999 & m_dToleranceAngle != 0)
		{
			while (true)
			{
				double tmpScore = -999;
				...
				bool isMatch = _FindMatch_SpecificAngle(iTopLayer, vecLayerScore, vecMatSrcPyr, tmpScore, pTemplData, tmpAngle);

				if (!isMatch) //找不到Chip直接跳過
					break;
...
#pragma region 由下邊界往上找

		if (m_dToleranceAngle != 0)
		{
			score = -999;

			while (true)
			{
				bool isMatch = _FindMatch_SpecificAngle(iTopLayer, vecLayerScore, vecMatSrcPyr, score, pTemplData, _lowerBound);
		...
		if (score != -999 & m_dToleranceAngle != 0)
		{
			while (true)
			{
				...
				{
					_lowerBound = tmpAngle;
					score = tmpScore;

					if (_lowerBound < -m_dToleranceAngle)
					{
						_lowerBound = m_dToleranceAngle;
						break;
					}
...
#pragma endregion
	...
	dAngleStep = atan(2.0 / max(pTemplData->vecPyramid[0].cols, pTemplData->vecPyramid[0].rows)) * R2D;

	vector<double> vecAngles;

	if (abs(_upperBound - _lowerBound) < VISION_TOLERANCE)
		vecAngles.push_back(0.0);
	else
	{
		for (double dAngle = _lowerBound; dAngle < _upperBound; dAngle += dAngleStep)
...
bool CMatchTool::_FindMatch_SpecificAngle(int iTopLayer, vector<double> vecLayerScore, vector<Mat> vecMatSrcPyr, double& result, s_TemplData* pTemplData, double angle)
{
	int iTopSrcW = vecMatSrcPyr[iTopLayer].cols, iTopSrcH = vecMatSrcPyr[iTopLayer].rows;
	Point2f ptCenter((iTopSrcW - 1) / 2.0f, (iTopSrcH - 1) / 2.0f);
	...
	Mat matRotatedSrc, matR = getRotationMatrix2D(ptCenter, angle, 1);
	Mat matResult;
	Point ptMaxLoc;
	double dValue, dMaxVal;
	double dRotate = clock();
...
bool CMatchTool::Match(Mat Img, vector<s_SingleTargetMatch>& result)
{
	...
	vector<s_MatchParameter>vecPossibleResult;

	for (int ilayer = iTopLayer; ilayer >= iStopLayer; ilayer--)
	{

		if (vecVecMatchParameter.size() == 0)
		{
			_RoughMatch(ilayer, vecLayerScore, vecMatSrcPyr, vecVecMatchParameter, pTemplData);
			//有初步定位結果 再修正細節下一個階層的金字塔再修正
		}
		else
		{

			for (int i = 0; i < (int)vecVecMatchParameter.size(); i++)
			{
				vector<s_MatchParameter>vecMatchParameter;
				vecMatchParameter = vecVecMatchParameter[i];
				vecVecMatchParameter[i].clear();

				for (int j = 0; j < (int)vecMatchParameter.size(); j++)
				{
					Mat matResult, matRotatedSrc;
					double dMaxValue = 0;
					Point ptMaxLoc;

					Point2f ptLT = ptRotatePt2f(vecMatchParameter[j].pt, ptCenter, 0);
					...
					s_MatchParameter obj = s_MatchParameter(ptMaxLoc, dMaxValue, vecAngles[j]);

					Point2f ptSrcCenter((vecMatSrcPyr[ilayer].cols - 1) / 2.0f, (vecMatSrcPyr[ilayer].rows - 1) / 2.0f);

					Point2f ptPaddingLT = ptRotatePt2f(ptLT * 2, ptSrcCenter, 0) - Point2f(3, 3);// Point2f(3, 3) 哪裡來的 原理未知 

					Point2f pt(obj.pt.x + ptPaddingLT.x, obj.pt.y + ptPaddingLT.y);
					//再旋轉
					pt = ptRotatePt2f(pt, ptSrcCenter, 0);

					vecMatchParameter[j].pt = pt;
					if (ilayer == iStopLayer)
					{
						vecMatchParameter[j].pt = pt * (iStopLayer == 0 ? 1 : 2);
						vecPossibleResult.push_back(vecMatchParameter[j]);
					}
					else
					{
						if (dMaxValue > vecLayerScore[ilayer])
							vecVecMatchParameter[i].push_back(vecMatchParameter[j]);
					}
				}
	...
	FilterWithScore(&vecPossibleResult, m_dScore);

	int iDstW = pTemplData->vecPyramid[iStopLayer].cols * (iStopLayer == 0 ? 1 : 2);
	int iDstH = pTemplData->vecPyramid[iStopLayer].rows * (iStopLayer == 0 ? 1 : 2);

	for (int i = 0; i < (int)vecPossibleResult.size(); i++)
	{
		Point2f ptLT, ptRT, ptRB, ptLB;
		double dRAngle = -vecPossibleResult[i].dMatchAngle * D2R;
		ptLT = vecPossibleResult[i].pt;
		...
		//紀錄旋轉矩形
		vecPossibleResult[i].rectR = RotatedRect(ptLT, ptRT, ptRB);
	}
	FilterWithRotatedRect(&vecPossibleResult, CV_TM_CCOEFF_NORMED, m_dMaxOverlap);
	//最後濾掉重疊


	//根據分數排序
	sort(vecPossibleResult.begin(), vecPossibleResult.end(), compareScoreBig2Small);
	m_vecSingleTargetData.clear();

	int iMatchSize = (int)vecPossibleResult.size();

	if (vecPossibleResult.size() == 0)
		return false;
	...
	for (int i = 0; i < iMatchSize; i++)
	{
		...
		sstm.ptLT = vecPossibleResult[i].pt;
		...
		sstm.dMatchedAngle = -vecPossibleResult[i].dMatchAngle;
		sstm.dMatchScore = vecPossibleResult[i].dMatchScore;

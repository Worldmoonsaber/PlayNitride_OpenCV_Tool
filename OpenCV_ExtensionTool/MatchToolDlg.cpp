...
bool CMatchToolDlg::Match ()
{
	...
	std::cout << "iTopLayer:: " << iTopLayer << endl;
	...
	//for (int i = 0; i < vecMatSrcPyr.size(); i++)
	//{
	//	Mat img = vecMatSrcPyr[i];
	//	Mat pattern = pTemplData->vecPyramid[i];
	...
	for (int i = 0; i < iSize; i++)
	{
		...
		MatchTemplate(vecMatSrcPyr[iTopLayer], pTemplData, matResult, iTopLayer, false);
...
void CMatchToolDlg::CCOEFF_Denominator (cv::Mat& matSrc, s_TemplData* pTemplData, cv::Mat& matResult, int iLayer)
{
	...
	double maxv;
	minMaxLoc(matResult, 0, &maxv, 0, 0);
	cout << "maxvalue=" << maxv << endl;

	matResult = matResult / maxv;

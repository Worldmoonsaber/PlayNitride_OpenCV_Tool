...
struct s_SingleTargetMatch
{
	...
	Point2d ptLT_New, ptRT_New, ptRB_New, ptLB_New, ptCenter_New;
...
public:
	...
	static bool CheckOverlapWithRotatedRect(vector<RotatedRect> vecRef,Size2f sz, s_SingleTargetMatch vec, int iMethod, double dMaxOverLap);

private:
	void _RoughMatch(int iTopLayer, vector<double> vecLayerScore, vector<Mat> vecMatSrcPyr, vector<vector<s_MatchParameter>>& vecVecMatchParameter, s_TemplData* pTemplData);
	bool _FindMatch_SpecificAngle(int iTopLayer, vector<double> vecLayerScore, vector<Mat> vecMatSrcPyr, double& result, s_TemplData* pTemplData,double angle);
	...
	static void SortPtWithCenter (vector<Point2f>& vecSort);
